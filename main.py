...
with col3:
        ...
        arxh = st.date_input("Ημερομηνία Έναρξης Άδειας",format="DD/MM/YYYY")
        hmer_gnomateyshs = st.date_input("Ημερομηνία Ιατρικής Γνωμάτευσης",format="DD/MM/YYYY")
...
with col4:
        ...
        st.text_input("Ημερομηνία Λήξης Άδειας", value=telos.strftime('%d/%m/%Y'), disabled=True)
...
if submitted or st.session_state.form_done:
    if not protocollo_adeias.strip():  
        st.warning("⚠️ Παρακαλώ συμπληρώστε τον Αριθμό Πρωτοκόλλου για να συνεχίσετε.")
        st.stop()  
    ...
    if not template_path:
        ...
        try:
            ...
            context = {
                ...
                'arxh': full_enarksi_text,
                'telos': full_lhxh_text,
